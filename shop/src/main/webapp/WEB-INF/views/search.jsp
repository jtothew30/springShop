...
<%-- 						<ul
							class="mobile-product-filters vertical menu show-for-small-only"
							data-accordion-menu>
							<li><a href="#">
									<h2>Products</h2>
							</a>
								<ul class="vertical menu" data-accordion-menu>
									<li class="product-filters-tab"><a href="#">Category</a>
										<ul class="categories-menu menu vertical nested is-active">
											<a href="#" class="clear-all" id="category-clear-all">Clear
												All</a>
											<c:forEach items="${cate2List}" var="i" varStatus="p">
												<li><input class="category-clear-selection chkboxCate2"
													name="chkboxCate2" onchange="boxtoggle();" type="checkbox"
													value="${i}"> <label
													for="category-checkbox${p.index+1}">${i}</label></li>
											</c:forEach>
										</ul></li>
									<li class="product-filters-tab"><a href="#">Option</a>
										<ul class="categories-menu menu vertical nested is-active">
											...
											<c:forEach items="${cate3List}" var="op" varStatus="i">
												<li><input class="chkboxCate3" name="chkboxCate3"
													onchange="boxtoggle();" type="checkbox" value="${op}"> <label
													for="size-checkbox${i.index+1}">${op}</label></li>
											</c:forEach>
										</ul></li>
						...
						<ul class="vertical menu hide-for-small-only" data-accordion-menu>
							<li class="product-filters-tab"><a href="#">Category</a>
								<ul class="categories-menu menu vertical nested is-active">
									...
									<c:forEach items="${cate2List}" var="i">
										<li>
										<input class="chkboxCate2" onchange="boxtoggle();" name="chkboxCate2" type="checkbox" value="${i}">
							...
							<li class="product-filters-tab"><a href="#">Size</a>
								<ul class="categories-menu menu vertical nested is-active">
									...
									<c:forEach items="${cate3List}" var="op" varStatus="i">
										<li>
										<input class="chkboxCate3" onchange="boxtoggle();" name="chkboxCate3" type="checkbox" value="${op}"> 
										<label for="size-checkbox${i.index+1}">${op}</label>
										</li>
									</c:forEach>
								</ul></li>
							<li class="product-filters-tab"><a href="#">Color</a>
						...
						<div class="product-card">
							<div class="product-card-thumbnail">
								<a href="proboard/product.do?pbno=${b.pbno}"> 
									<img src="${pageContext.request.contextPath}${b.path}/1.jpg" width="180" height="180" />
								</a>
							</div>
							<h2 class="product-card-title">
								<a href="proboard/product.do?pbno=${b.pbno}">${b.title}</a>
							</h2>
							<span class="product-card-desc">${b.options}</span>
							<span class="product-card-price"> 
							<fmt:formatNumber value="${b.price}" type="currency" currencySymbol="￦" /></span>
							<c:if test="${b.price < b.event}">
	...
	<script type="text/javascript">
		...
		// checkbox 받아온값 처리하기
		$(document).ready(function() {
			var list2 = <c:out value="${paging.optionKwd}" escapeXml="false"/>
			console.log(list2);
			if(list2=="") return;
			var arr = JSON.parse(list2);
			var list2 = arr.listCate2;
			var list3 = arr.listCate3;
			console.log(list2);
			console.log(list3);
			$('.chkboxCate2').each(function() {
		...
		function chk() {
			...
			$('.chkboxCate2:checked').each(function() {
				if (listCate2.indexOf($(this).val()) < 0) {
					listCate2.push($(this).val());
				}
			});
			var listCate3 = [];
			$('.chkboxCate3:checked').each(function() {
				if (listCate3.indexOf($(this).val()) < 0) {
